...
function addController(router, path, options, controller, description) {
  ...
  if (description !== false) {
    ...
    var path = _pjoin(router.path, path);
    router._urls.push([options.method, path, title]);
    router._paths[path] = router._paths[path] || {
      path: path,
      ops: []
    };
    var o = {
    ...
    if (controller.docs) {
      o.responses = controller.docs.responses || [{
        'code': '200', 'description': 'Ok!'
      }];

      o.responses = o.responses.map(r => {
        r = extend({}, r);
        if (r.schema && r.schema.indexOf('{model.jsonSchema') >= 0) {
          r.schema = jsonSchema(router.model);
          delete r.schema.$schema;
          r.schema = JSON.stringify(r.schema, '', '  ');
        } else {
          r.schema = '"' + r.schema + '"'
        }
        return r;
