...
const ensureMethodObj = helpers.ensureMethodObj;
const normalizeReadonly = helpers.normalizeReadonly;
const camel2giffen = helpers.camel2giffen;
const flat = helpers.flat;
const addController = helpers.addController;
const exposeMethods = helpers.exposeMethods;
 ...
 * @param  {String|Object|Array}    [options.readonly]  the list of fields (paths) that couldn't be modified directly via create and update
...
ModelAPIRouter.prototype.__applyOptions = function(options) {
  ...
  return {
    ...
    readonly: normalizeReadonly(options.readonly) || { _id: true },
...
ModelAPIRouter.prototype.option = function(method, name) {
  const methodOptions = this.apiOptions[method];
  if (methodOptions && typeof(methodOptions[name]) !== 'undefined') {
